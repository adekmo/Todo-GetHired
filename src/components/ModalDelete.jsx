...
const ModalDelete = ({show, onClose, detail}) => {
    ...
    const handleDelete = async () => {
        ...
        fetch('https://todo.api.devcode.gethired.id/activity-groups/' + detail.id, {
            method: 'DELETE',
        }).then(() => {
            onClose();
            navigate('/');
        ...
        <div className='bg-white/10 bg-opacity-10 backdrop-blur-sm w-full h-full self-start flex justify-center absolute left-0'>
            <div data-cy="modal-delete" className='w-[490px] h-[355px] bg-white flex flex-col justify-around items-center sort-animation'>
                ...
                <div data-cy="modal-delete-title" className='w-[365px] h-[54px]'>
                    <p className='font-poppins font-medium text-[18px] leading-[27px] text-[#111111]'>Apakah anda yakin menghapus activity <span className='font-poppins font-bold text-[18px] leading[27px] '>{detail.title}</span></p>
                </div>
