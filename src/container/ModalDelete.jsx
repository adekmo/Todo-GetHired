...
const ModalDelete = ({show, onClose, detail, getActivity, handleAlert}) => {

    const handleDelete = () => {
        if(detail.created_at){
            const url = 'https://todo.api.devcode.gethired.id/activity-groups/' + detail.id;
            fetch(url, {
                method: 'DELETE',
            }).then(() => {
                onClose();
            ...
            const url = 'https://todo.api.devcode.gethired.id/todo-items/' + detail.id;
            fetch(url, {
                method: 'DELETE',
            }).then(() => {
                onClose();
    ...
    if(!show) return null;
    return (
        <div className='top-[290px] bg-white/10 bg-opacity-10 backdrop-blur-sm w-full h-full self-start flex justify-center absolute left-0'>
            ...
            <div data-cy="modal-delete" className='w-[490px] h-[355px] bg-white flex flex-col justify-around items-center sort-animation'>
                ...
                <div data-cy="modal-delete-title" className='w-[365px] h-[54px]'>
                    <p className='font-poppins font-medium text-[18px] leading-[27px] text-[#111111]'>Apakah anda yakin menghapus {detail.created_at ? 'activity' : 'List Item' } <span className='font-poppins font-bold text-[18px] leading[27px] '>{detail.title}</span></p>
                </div>
